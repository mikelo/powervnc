...
//  VncViewer viewer;
  ...
  private final static long VNC_EVENTS = AWTEvent.KEY_EVENT_MASK
  ...
  public VncCanvas(VNCMain v) throws IOException {

    enableEvents(VNC_EVENTS);
    ...
    inputEnabled = false;
    if (!viewer.getOptions().isViewOnly()) {
      enableInput(true);
    ...
    //if (version.startsWith("1.4")) {
      try {
        Class[] params = new Class[] {
            boolean.class};
        VncCanvas.class.getMethod("setFocusable", params).invoke(this,
            new Object[] {new Boolean(true)});
        VncCanvas.class.getMethod("setFocusTraversalKeysEnabled",
                                  params).invoke(this,
                                                 new Object[] {new Boolean(false)});
      }
      catch (Exception e) {
        System.err.println(
            "VncCanvas: unable to reset focus handling for java version "
            + version);
        e.printStackTrace();
      }
    }
...
//  }
  ...
  public void setPixelFormat() throws IOException {
    if (viewer.getOptions().isEightBitColors()) {
  ...
  void resizeDesktopFrame() {
    ...
    if (viewer.getScreenSizePolicy() ==
      ...
      if (viewer.getFrame() != null && viewer.isFrameResizeable()) {
    ...
    /* else {
       setSize(viewer.getScreenSize());
       viewer.getFrame().pack();
       viewer.getFrame().setSize(viewer.getScreenSize());
       repaint();
       //viewer.getFrame().invalidate();
       //viewer.revalidate();
       //revalidate();
     }*/

    // Recenter the viewer upon resize completion
    UIUtil.positionComponent(UIUtil.CENTER, viewer);
  }
    ...
    while (true) {
      ...
      switch (msgType) {
        case RfbProto.FramebufferUpdate:
          ...
          if (viewer.getDeferUpdateRequests() > 0 &&
          ...
          if (viewer.getOptions().isEightBitColors() != (bytesPixel == 1)) {
            setPixelFormat();
            fullUpdateNeeded = true;
          }
        ...
        case RfbProto.ServerCutText:
          ...
          viewer.getClipboard().setCutText(s);
          break;
  ...
  void handleRawRect(int x, int y, int w, int h, boolean paint) throws
      IOException {
  ...
  synchronized void
      handleCursorShapeUpdate(int encodingType,
                              int xhot, int yhot, int width, int height) throws
      IOException {
    ...
    if (viewer.getOptions().isIgnoreCursorUpdates()) {
    ...
    repaint(viewer.getDeferCursorUpdates(),
