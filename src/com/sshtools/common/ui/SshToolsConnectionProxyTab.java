...
public class SshToolsConnectionProxyTab
  ...
  public void setConnectionProfile(SshToolsConnectionProfile profile) {
    ...
    if (profile.getTransportProvider() ==
        SshToolsConnectionProfile.USE_HTTP_PROXY) {
      httpProxy.setSelected(true);
    }
    else if (profile.getTransportProvider() ==
             SshToolsConnectionProfile.USE_SOCKS4_PROXY) {
      socks4Proxy.setSelected(true);
    }
    else if (profile.getTransportProvider() ==
             SshToolsConnectionProfile.USE_SOCKS5_PROXY) {
  ...
  public void applyTab() {
    if (httpProxy.isSelected()) {
      ...
      profile.setTransportProvider(SshToolsConnectionProfile.
                                   USE_STANDARD_SOCKET);
    }
