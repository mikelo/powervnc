...
public class TerminalPanel
  ...
  public TerminalPanel(VDUBuffer buffer, Font font) {
	...
	if(fnts[i].equals("Lucida Sans Typewriter")) lucidaTypewriterFound=true;
	if(fnts[i].equals("Courier New")) courierNewFound=true;
    }
	...
	if(System.getProperty("os.name").startsWith("Windows")) {
		...
		if(lucidaTypewriterFound) {
		    setFont(new Font("Lucida Sans Typewriter", Font.PLAIN, 11));
		} else {
  ...
  public synchronized void setBounds(int x, int y, int w, int h) {
    ...
    switch (resizeStrategy) {
      case RESIZE_SCREEN:
        ...
        if ((buffer.width  != (w / charWidth)) ||
            (buffer.height != (h / charHeight))) {
  ...
  public synchronized void mouseDragged(MouseEvent evt) {
    if (buttonCheck(evt.getModifiers(), MouseEvent.BUTTON1_MASK)
        ...
        evt.getModifiers() == 0)) {
	if(((new Date()).getTime() - lastRelease)>10) inDrag = true;
      int xoffset = (super.getSize().width - (buffer.width * charWidth)) / 2;
