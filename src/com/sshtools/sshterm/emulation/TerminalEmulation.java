...
public class TerminalEmulation
  ...
  public TerminalEmulation(String term, int width, int height) throws
      IOException {
    super(width, height);
  ...
  public void keyPressed(int keyCode, char keyChar, int modifiers) {
    if (debug > 0) {
      System.out.println("keyPressed(" + keyCode + "," + keyChar + ","
                         + modifiers);
    }
  ...
  public void keyTyped(int keyCode, char keyChar, int modifiers) {
    if (debug > 0) {
      System.out.println("keyTyped(" + keyCode + "," + keyChar + ","
                         + modifiers);
    }
    //  If there is a scrollbar, scroll to the end
    if ( (display != null) && (display.getScrollBar() != null)) {
      if (display.getScrollBar().getValue() != display.getScrollBar()
          .getMaximum()) {
        display.getScrollBar().setValue(display.getScrollBar()
                                        .getMaximum());
      }
    }
    boolean control = (modifiers & VDUInput.KEY_CONTROL) != 0;
    ...
    if(control && keyChar>='a' && keyChar<='z') keyChar=(char)(keyChar-96);
    if(control && shift && keyChar>='A' && keyChar<='Z') keyChar=(char)(keyChar-64);
    boolean alt = (modifiers & VDUInput.KEY_ALT) != 0;
    if (keyChar == '\t') {
      if (shift) {
    ...
    if (! ( (keyChar == 8) || (keyChar == 127) || (keyChar == '\r')
           || (keyChar == '\n'))) {
      write("" + keyChar);
      return;
    }
  ...
  private void putChar(char c, boolean doshowcursor) {
    ...
    if (debug > 4) {
      System.out.println("putChar(" + c + " [" + ( (int) c) + "]) at R="
                         + R + " , C=" + C + ", columns=" + columns + ", rows=" +
                         rows);
    }
    markLine(R, 1);
    if (c > 255) {
    ...
    switch (term_state) {
      case TSTATE_DATA:
        ...
        if (!useibmcharset) {
          ...
          switch (c) {
            ...
            case IND:
              if (debug > 2) {
                System.out.println("IND at " + R + ", tm is " + tm
                                   + ", bm is " + bm);
              }
              if ( (R == bm) || (R == (rows - 1))) {
        ...
        switch (c) {
          ...
          case '\n':
            ...
            if (debug > 3) {
              System.out.println("R= " + R + ", bm " + bm + ", tm=" + tm
                                 + ", rows=" + rows);
            }
            if (!vms) {
                ...
                switch (gx[thisgl]) {
                  case '0':
                    ...
                    if (terminalID.equals("scoansi")
                      ...
                      for (int i = 0; i < scoansi_acs.length();
                           i += 2) {
                  ...
                  case 'B':
                    ...
                    break;
                  default:
                    System.out.println("Unsupported GL mapping: "
                                       + gx[thisgl]);
                    break;
                }
              }
              if (!mapped && ( (c >= '\u0080') && (c <= '\u00ff'))) {
                switch (gx[gr]) {
                  ...
                  case 'B':
                    ...
                    break;
                  default:
                    System.out.println("Unsupported GR mapping: "
                                       + gx[gr]);
                    break;
                }
              }
      ...
      case TSTATE_SETG0:
        if ( (c != '0') && (c != 'A') && (c != 'B') && (c != '<')) {
          System.out.println("ESC ( " + c + ": G0 char set?  ("
                             + ( (int) c) + ")");
        }
        else {
          if (debug > 2) {
      ...
      case TSTATE_SETG1:
        if ( (c != '0') && (c != 'A') && (c != 'B') && (c != '<')) {
          System.out.println("ESC ) " + c + " (" + ( (int) c)
                             + ") :G1 char set?");
        }
        else {
          if (debug > 2) {
          ...
          case 'i':
            ...
            switch (DCEvars[0]) {
              case 1:
                if (debug > 1) {
                  System.out.println(
                      "CSI ? 1 i : Print line containing cursor");
                }
                break;
              case 4:
                if (debug > 1) {
                  System.out.println(
                      "CSI ? 4 i : Start passthrough printing");
                }
                break;
              case 5:
                if (debug > 1) {
                  System.out.println(
                      "CSI ? 4 i : Stop passthrough printing");
                }
                break;
            }
            break;
      ...
      case TSTATE_CSI_EX:
        ...
        switch (c) {
          case ESC:
            ...
            break;
          default:
            System.out.println("Unknown character ESC[! character is "
                               + (int) c);
            break;
        }
        break;
      case TSTATE_CSI_TICKS:
        ...
        switch (c) {
          case 'p':
            System.out.println("Conformance level: " + DCEvars[0]
                               + " (unsupported)," + DCEvars[1]);
            if (DCEvars[0] == 61) {
              output8bit = false;
              break;
            }
        ...
        switch (c) {
          ...
          */
          case '~':
            ...
            break;
          default:
            System.out.println("UNKNOWN Status Display code " + c
                               + ", with Pn=" + DCEvars[0]);
            break;
        }
        break;
      case TSTATE_CSI:
        ...
        switch (c) {
          ...
          case 'h':
            switch (DCEvars[0]) {
              ...
              case 20:
                ...
                break;
              default:
                System.out.println("unsupported: ESC [ " + DCEvars[0]
                                   + " h");
                break;
            }
            if (debug > 1) {
          ...
          case 'i':
            ...
            switch (DCEvars[0]) {
              case 0:
                if (debug > 1) {
                  System.out.println(
                      "CSI 0 i:  Print Screen, not implemented.");
                }
                break;
              case 4:
                if (debug > 1) {
                  System.out.println(
                      "CSI 4 i:  Enable Transparent Printing, not implemented.");
                }
                break;
              case 5:
                if (debug > 1) {
                  System.out.println(
                      "CSI 4/5 i:  Disable Transparent Printing, not implemented.");
                }
                break;
              default:
                System.out.println("ESC [ " + DCEvars[0]
          ...
          case 'H':
            ...
            if (debug > 2) {
              System.out.println("ESC [ " + DCEvars[0] + ";" + DCEvars[1]
                                 + " H, moveoutsidemargins " +
                                 moveoutsidemargins);
          ...
          case 'X': {
            ...
            if (debug > 1) {
              System.out.println("ESC [ " + DCEvars[0] + " X, C=" + C
                                 + ",R=" + R);
            }
            if (toerase == 0) {
              toerase = 1;
          ...
          case 'P':
            if (debug > 1) {
              System.out.println("ESC [ " + DCEvars[0] + " P, C=" + C
                                 + ",R=" + R);
            }
            if (DCEvars[0] == 0) {
              DCEvars[0] = 1;
              ...
              switch (DCEvars[i]) {
                ...
                */
                ...
                case 49:
                  ...
                  break;
                default:
                  System.out.println("ESC [ " + DCEvars[i]
                                     + " m unknown...");
                  break;
              }
              if (debug > 3) {
  ...
  class TerminalOutputStream
    ...
    public void write(byte[] buf, int off, int len) throws IOException {
      synchronized (lock) {
        TerminalEmulation.this.putString(new String(buf, off, len,
            "latin1"));
      }
    }
  }
