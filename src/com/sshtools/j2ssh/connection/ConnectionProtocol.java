...
public class ConnectionProtocol
  ...
  private Long getChannelId() {
    ...
    /* David Spence:
     * There is a race condition (with consequent deadlock) here as the message store does not give ordered delivery of messages when
     * getMessage() and registerMessage() are used at the same time.  Therefore the receiving of the SSH_MSG_CHANNEL_OPEN_CONFIRMATION
     * message has been moved to the normal message handler.
     */
	...
	public void completeConnection(SshMsgChannelOpenConfirmation conf) {  ///must be called sychronized to the owning class!!
	    try {
		synchronized(activeChannels) {
		    activeChannels.put(channelId, channel);
		    
		    log.debug("Initiating channel");
		    channel.init(ConnectionProtocol.this, channelId.longValue(),
	...
	public synchronized boolean tail() throws InterruptedException { // wait for outcome
	    wait();
	    return result;
	}
    }
    ...
    private synchronized void onMsgChannelOpenConfirmation(SshMsgChannelOpenConfirmation conf) {
	UnopenedChannel uoc = null;
	Long id = new Long(conf.getRecipientChannel());
	synchronized(unopenedChannels) {
	    uoc = (UnopenedChannel) unopenedChannels.get(id);
	}
	if(uoc==null) {
    ...
    private synchronized void onMsgChannelOpenFailure(SshMsgChannelOpenFailure fail) {
	UnopenedChannel uoc = null;
 ...
 /* protected int[] getAsyncMessageFilter() {
  ...
  public void onMessageReceived(SshMessage msg) throws IOException {
    ...
    switch (msg.getMessageId()) {
      ...
      case SshMsgChannelOpenConfirmation.SSH_MSG_CHANNEL_OPEN_CONFIRMATION: {
        onMsgChannelOpenConfirmation( (SshMsgChannelOpenConfirmation) msg);

        break;
      }

      case SshMsgChannelOpenFailure.SSH_MSG_CHANNEL_OPEN_FAILURE: {
        onMsgChannelOpenFailure( (SshMsgChannelOpenFailure) msg);
  ...
  protected void onServiceInit(int startMode) throws IOException {
    ...
    transport.getMessageStore().registerMessage(SshMsgChannelOpenConfirmation.
                                 SSH_MSG_CHANNEL_OPEN_CONFIRMATION,
                                 SshMsgChannelOpenConfirmation.class, this);

    transport.getMessageStore().registerMessage(SshMsgChannelOpenFailure.
                                 SSH_MSG_CHANNEL_OPEN_FAILURE,
                                 SshMsgChannelOpenFailure.class, this);
  ...
  protected void sendChannelOpenConfirmation(Channel channel) throws
      IOException {
    SshMsgChannelOpenConfirmation msg = new SshMsgChannelOpenConfirmation(
        channel
