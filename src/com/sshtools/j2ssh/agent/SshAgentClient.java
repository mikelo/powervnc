...
public class SshAgentClient {
  ...
  SshAgentClient(boolean isForwarded, String application, Socket socket) throws
      IOException {
    log.info("New SshAgentClient instance created");
    this.socket = socket;
  ...
  public static SshAgentClient connectLocalAgent(String application,
      ...
      AgentNotAvailableException, IOException {
    try {
   ...
   * @throws IOException if an IO error occurs
   */
  public static Socket connectAgentSocket(String location) throws
      AgentNotAvailableException, IOException {
    try {
      if (location == null) {
        throw new AgentNotAvailableException();
  ...
  public void addKey(SshPrivateKey prvkey, SshPublicKey pubkey,
                     String description, KeyConstraints constraints) throws
      IOException {
   ...
   * @throws IOException if an IO error occurs
   */
  public void deleteKey(SshPublicKey key, String description) throws
      IOException {
  ...
  protected void sendForwardingNotice() throws IOException {
    ...
    SshAgentForwardingNotice msg = new SshAgentForwardingNotice(addr
        .getHostName(), addr.getHostAddress(), 22);
