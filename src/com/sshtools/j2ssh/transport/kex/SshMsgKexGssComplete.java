...
public class SshMsgKexGssComplete
    extends SshMessage {
  /**  */
  protected final static int SSH_MSG_KEXGSS_COMPLETE = 32;
  ...
  /**
   * Creates a new SshMsgKexGssContinue object.  token=null means no token.
   *
   * @param token
   */
  public SshMsgKexGssComplete(byte[] token, byte[] MIC, BigInteger f) {
    super(SSH_MSG_KEXGSS_COMPLETE);
    this.f = f;
    this.MIC = MIC;
    this.token = token;
  }
