...
public class TransportProtocolClient
   ...
   */
  public TransportProtocolClient(HostKeyVerification hosts) throws
  ...
  /**
   * Redefined so we can exclude gss methods on re-exchange.
   *
   * @return
   *
   * @throws IOException
  ...
  protected SshMsgKexInit createLocalKexInit() throws IOException {
    return new SshMsgKexInit(properties, firstExch);
  }
  ...
  protected void performKeyExchange(SshKeyExchange kex,
    ...
    // Verify the hoskey
    if(kex.getHostKey()!=null || kex.getGSSContext()==null) {
   ...
   * @throws TransportProtocolException
   */
  protected boolean verifyHostKey(byte[] key, byte[] sig, byte[] sigdata) throws
      TransportProtocolException {
