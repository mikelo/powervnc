...
public class SshMsgKexInit
  ...
  public SshMsgKexInit(SshConnectionProperties props, boolean firstExch) {
    ...
    supportedKex = SshKeyExchangeFactory.getSupportedKeyExchanges(firstExch);
    if(supportedKex.contains(pref1)) {
	supportedKex = sortAlgorithmList(SshKeyExchangeFactory
					 .getSupportedKeyExchanges(firstExch),
					 props.getPrefKex());
    } else {
	supportedKex = sortAlgorithmList(SshKeyExchangeFactory
					 .getSupportedKeyExchanges(firstExch),
					 SshKeyExchangeFactory
					 .getDefaultKeyExchange(firstExch));
