...
public class ForwardingConfiguration {
  ...
  /**  */
  protected ForwardingConfigurationMonitor monitor = new
  ...
  public ForwardingSocketChannel createForwardingSocketChannel(String type,
    ...
    if (type.equals(ForwardingChannel.LOCAL_FORWARDING_CHANNEL)) {
      channel = new ForwardingSocketChannel(type, name, hostToConnect,
                                            portToConnect, originatingHost,
                                            originatingPort);
    }
    else {
      channel = new ForwardingSocketChannel(type, name, getAddressToBind(),
                                            getPortToBind(), originatingHost,
                                            originatingPort);
    }
   ...
   * @throws ForwardingConfigurationException
   */
  public ForwardingSocketX11Channel createForwardingSocketX11Channel(String type,
      String hostToConnect, int portToConnect, String originatingHost,
      int originatingPort, String authType, String fakeAuthData, String realAuthData) throws ForwardingConfigurationException {
    if (state.getValue() == StartStopState.STOPPED) {
      throw new ForwardingConfigurationException(
          "The forwarding has been stopped");
    }
    ...
    ForwardingSocketX11Channel channel;

    if (type.equals(ForwardingChannel.LOCAL_FORWARDING_CHANNEL)) {
      channel = new ForwardingSocketX11Channel(type, name, hostToConnect,
                                            portToConnect, originatingHost,
                                            originatingPort, authType, fakeAuthData, realAuthData);
    }
    else {
      channel = new ForwardingSocketX11Channel(type, name, getAddressToBind(),
                                            getPortToBind(), originatingHost,
                                            originatingPort, authType, fakeAuthData, realAuthData);
    }
   ...
   * @throws ForwardingConfigurationException
   */
  public ForwardingUNIXSocketChannel createForwardingUNIXSocketChannel(String type,
      String hostToConnect, int portToConnect, String originatingHost,
      int originatingPort) throws ForwardingConfigurationException {
    if (state.getValue() == StartStopState.STOPPED) {
      throw new ForwardingConfigurationException(
    ...
    ForwardingUNIXSocketChannel channel;

    if (type.equals(ForwardingChannel.LOCAL_FORWARDING_CHANNEL)) {
      channel = new ForwardingUNIXSocketChannel(type, name, hostToConnect,
                                            portToConnect, originatingHost,
                                            originatingPort);
    }
    else {
      channel = new ForwardingUNIXSocketChannel(type, name, getAddressToBind(),
                                            getPortToBind(), originatingHost,
                                            originatingPort);
    }
   ...
   * @throws ForwardingConfigurationException
   */
  public ForwardingUNIXSocketX11Channel createForwardingUNIXSocketX11Channel(String type,
      String hostToConnect, int portToConnect, String originatingHost,
      int originatingPort, String authType, String fakeAuthData, String realAuthData) throws ForwardingConfigurationException {
    if (state.getValue() == StartStopState.STOPPED) {
      throw new ForwardingConfigurationException(
    ...
    ForwardingUNIXSocketX11Channel channel;

    if (type.equals(ForwardingChannel.LOCAL_FORWARDING_CHANNEL)) {
      channel = new ForwardingUNIXSocketX11Channel(type, name, hostToConnect,
                                            portToConnect, originatingHost,
                                            originatingPort, authType, fakeAuthData, realAuthData);
    }
    else {
      channel = new ForwardingUNIXSocketX11Channel(type, name, getAddressToBind(),
                                            getPortToBind(), originatingHost,
                                            originatingPort, authType, fakeAuthData, realAuthData);
    }
  ...
  public ForwardingIOChannel createForwardingIOChannel(String type,
    ...
    if (type.equals(ForwardingChannel.LOCAL_FORWARDING_CHANNEL)) {
      channel = new ForwardingIOChannel(type, name, getHostToConnect(),
                                        getPortToConnect(), originatingHost,
                                        originatingPort);
    }
    else {
  ...
  public ForwardingBindingChannel createForwardingBindingChannel(
    ...
    if (type.equals(ForwardingChannel.LOCAL_FORWARDING_CHANNEL)) {
      ...
      channel = new ForwardingBindingChannel(type, name, getAddressToBind(),
                                             getPortToBind(), originatingHost,
                                             originatingPort);
    }
