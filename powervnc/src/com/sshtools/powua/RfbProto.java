...
class RfbProto {
  ...
  RfbProto(String h, int p, VNCMain v) throws IOException {
    viewer = v;
    host = h;
    port = p;

    if (viewer.getSocketFactory() == null) {
      sock = new Socket(host, port);
    }
    else {
      try {
        Class factoryClass = Class.forName(viewer.getSocketFactory());
...
//
  ...
  RfbProto(InputStream in, OutputStream out, VNCMain v) throws
      IOException {
    viewer = v;
  ...
  void writeClientInit() throws IOException {
    if (viewer.getOptions().isShareDesktop()) {
      os.write(1);
    }
    else {
      os.write(0);
  ...
  void writeSetPixelFormat(int bitsPerPixel, int depth, boolean bigEndian,
                           ...
                           int redShift, int greenShift, int blueShift) throws
      IOException {
    byte[] b = new byte[20];
  ...
  void writeFixColourMapEntries(int firstColour, int nColours,
                                int[] red, int[] green, int[] blue) throws
      IOException {
  ...
  void writePointerEvent(MouseEvent evt) throws IOException {
    ...
    int mask2 = 2;
    int mask3 = 4;
    if (viewer.getOptions().isReverseMouseButtons2And3()) {
      mask2 = 4;
      mask3 = 2;
    }
