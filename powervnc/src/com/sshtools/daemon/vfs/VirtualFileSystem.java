...
public class VirtualFileSystem
  ...
  static {
    try {
      vfsmounts = ( (PlatformConfiguration) ConfigurationLoader
                   .getConfiguration(PlatformConfiguration.class)).getVFSMounts();
      vfsroot = ( (PlatformConfiguration) ConfigurationLoader
                 .getConfiguration(PlatformConfiguration.class)).getVFSRoot();
    }
  ...
  public static void setPermissionHandler(VFSPermissionHandler
                                          permissionHandler) {
  ...
  private static String getVFSHomeDirectory(String username) throws
  ...
  public static boolean startsWithIgnoreCase(String str, String with) {
    return str.substring(0,
                         (with.length() > str.length()) ? str.length() :
                         with.length())
   ...
   */
  public static String translateNFSPath(String nfspath) throws
      FileNotFoundException {
  ...
  private static VFSMount getMount(String vfspath) throws FileNotFoundException,
      IOException {
    String vfshome = getVFSHomeDirectory(SshThread.getCurrentThreadUser());
    VFSMount m;
  ...
  public static String translateVFSPath(String vfspath, String vfscwd) throws
      FileNotFoundException {
        ...
        if (vfspath.startsWith(vfshome)) {
          ...
          if (vfspath.length() > vfshome.length()) {
            return translateCanonicalPath(nfshome
                                          + (nfshome.endsWith("/") ? "" : "/")
                                          +
                                          vfspath.substring(vfshome.length() +
                1),
                                          nfshome);
          }
          else {
   ...
   * @throws FileNotFoundException
   */
  public static String translateCanonicalPath(String path, String securemount) throws
      FileNotFoundException {
    try {
...
//    String realPath = path;
   ...
   * @throws IOException
   */
  public VFSPermission getVFSPermission(String path) throws
      FileNotFoundException, IOException {
   ...
   * @throws IOException
   */
  public SftpFile[] readDirectory(byte[] handle) throws InvalidHandleException,
  ...
  public byte[] openFile(String path, UnsignedInteger32 flags,
    ...
    if (!f.exists()) {
      if ( (flags.intValue() & NativeFileSystemProvider.OPEN_CREATE) ==
          NativeFileSystemProvider.OPEN_CREATE) {
      ...
      if ( ( (flags.intValue() & NativeFileSystemProvider.OPEN_CREATE) ==
            NativeFileSystemProvider.OPEN_CREATE)
          && ( (flags.intValue()
                & NativeFileSystemProvider.OPEN_EXCLUSIVE) ==
              NativeFileSystemProvider.OPEN_EXCLUSIVE)) {
    ...
    String mode = "r"
        +
        ( ( (flags.intValue() & NativeFileSystemProvider.OPEN_WRITE) ==
    ...
    if ( ( (flags.intValue() & NativeFileSystemProvider.OPEN_CREATE) ==
          NativeFileSystemProvider.OPEN_CREATE)
        &&
        ( (flags.intValue() & NativeFileSystemProvider.OPEN_TRUNCATE) ==
         NativeFileSystemProvider.OPEN_TRUNCATE)) {
  ...
  public byte[] readFile(byte[] handle, UnsignedInteger64 offset,
                         UnsignedInteger32 len) throws InvalidHandleException,
      EOFException, IOException {
    String shandle = new String(handle);
    ...
    if (openFiles.containsKey(shandle)) {
      ...
      if (obj instanceof OpenFile) {
        ...
        if ( (file.getFlags().intValue()
              & NativeFileSystemProvider.OPEN_READ) ==
            NativeFileSystemProvider.OPEN_READ) {
  ...
  public void writeFile(byte[] handle, UnsignedInteger64 offset, byte[] data,
                        int off, int len) throws InvalidHandleException,
      IOException {
    String shandle = new String(handle);
    ...
    if (openFiles.containsKey(shandle)) {
      ...
      if (obj instanceof OpenFile) {
        ...
        if ( (file.getFlags().intValue()
            ...
            NativeFileSystemProvider.OPEN_WRITE) {
          if ( (file.getFlags().intValue()
                & NativeFileSystemProvider.OPEN_APPEND) ==
              NativeFileSystemProvider.OPEN_APPEND) {
   ...
   * @throws IOException
   */
  public void renameFile(String oldpath, String newpath) throws
   ...
   * @throws FileNotFoundException
   */
  public void setFileAttributes(String path, FileAttributes attrs) throws
      PermissionDeniedException, IOException, FileNotFoundException {
   ...
   * @throws InvalidHandleException
   */
  public void setFileAttributes(byte[] handle, FileAttributes attrs) throws
      PermissionDeniedException, IOException, InvalidHandleException {
  }
   ...
   * @throws PermissionDeniedException
   */
  public SftpFile readSymbolicLink(String path) throws
      UnsupportedFileOperationException, FileNotFoundException,
   ...
   * @throws PermissionDeniedException
   */
  public void createSymbolicLink(String link, String target) throws
      UnsupportedFileOperationException, FileNotFoundException,
      IOException, PermissionDeniedException {
    throw new UnsupportedFileOperationException(
        "Symbolic links are not supported by the Virtual File System");
