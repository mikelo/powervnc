...
public class DirectoryListingTableModel
  ...
  public boolean isDirectory(int row) {
    ...
    synchronized (lock) {
      ...
      if(file.isLink()) {
	  try {
	      String realPath = sftp.symlinkTarget(file.getAbsolutePath());
	      FileAttributes fattr = sftp.stat(realPath);
	      int n = 0;
	      while (n<5 && fattr.isLink()) {
		  realPath = sftp.symlinkTarget(realPath);
		  fattr = sftp.stat(realPath);
		  n++;
	      }
