...
public class SftpSubsystemClient
   ...
   * @throws IOException
   */
  public synchronized FileAttributes getAttributes(String path) throws
      IOException {
   ...
   * @throws IOException
   */
  public synchronized FileAttributes getAttributes(SftpFile file) throws
      IOException {
    SubsystemMessage msg;
    UnsignedInteger32 requestId = nextRequestId();
  ...
  protected synchronized int readFile(byte[] handle,
                                      UnsignedInteger64 offset, byte[] output,
    ...
    try {
      ...
      if (reply instanceof SshFxpData) {
        ...
        if (status.getErrorCode().intValue() == SshFxpStatus.STATUS_FX_EOF) {
          ...
          throw new IOException( ( (SshFxpStatus) reply)
                                .getErrorMessage());
        }
      }
      else {
   ...
   * @throws IOException
   */
  public synchronized void renameFile(String oldpath, String newpath) throws
      IOException {
    UnsignedInteger32 requestId = nextRequestId();
  ...
  protected synchronized void writeFile(byte[] handle,
                                        UnsignedInteger64 offset, byte[] data,
                                        int off, int len) throws IOException {
    if (!handles.contains(handle)) {
   ...
   * @throws IOException
   */
  public synchronized String getSymbolicLinkTarget(String linkpath) throws
      IOException {
    UnsignedInteger32 requestId = nextRequestId();
   ...
   * @throws IOException
   */
  public synchronized void setAttributes(String path, FileAttributes attrs) throws
      IOException {
    UnsignedInteger32 requestId = nextRequestId();
   ...
   * @throws IOException
   */
  public synchronized void setAttributes(SftpFile file, FileAttributes attrs) throws
      IOException {
    if (!isValidHandle(file.getHandle())) {
   ...
   * @throws IOException
   */
  public void changePermissions(SftpFile file, String permissions) throws
      IOException {
   ...
   * @throws IOException
   */
  public void changePermissions(SftpFile file, int permissions) throws
      IOException {
    FileAttributes attrs = new FileAttributes(); //file.getAttributes();
   ...
   * @throws IOException
   */
  public void changePermissions(String filename, int permissions) throws
      IOException {
   ...
   * @throws IOException
   */
  public void changePermissions(String filename, String permissions) throws
      IOException {
    FileAttributes attrs = new FileAttributes();
    attrs.setPermissions(permissions);
    setAttributes(filename, attrs);
  ...
  public synchronized boolean initialize() throws IOException {
    ...
    if (reply instanceof SshFxpVersion) {
      ...
      log.info("Server responded with version " + String.valueOf(version));
    }

    return result;
  }

  private byte[] getHandleResponse(UnsignedInteger32 requestId) throws
      IOException {
    try {
      SubsystemMessage reply = messageStore.getMessage(requestId);
  ...
  private void getOKRequestStatus(UnsignedInteger32 requestId) throws
      IOException {
    try {
