...
public abstract class SshToolsApplicationClientApplet
  ...
  public void initApplet() throws IOException {
    ...
    // 
    // This value is here because we must provide some default.
    //
    port = PropertyUtil.stringToInt(getParameter(
        "sshapps.connection.port", ""), 2222);
    // ************************************************************************************
    ...
    showConnectionDialog = getParameter(
        "sshapps.connection.showConnectionDialog",
        "false").equalsIgnoreCase("true");
  ...
  public void startApplet() {
    ...
    if (disableHostKeyVerification) {
      ( (SshToolsApplicationClientPanel) applicationPanel)
          .setHostHostVerification(null);
      ( (SshToolsApplicationClientPanel) applicationPanel).application
          .removeAdditionalOptionsTab("Hosts");
    ...
    if (connectImmediately) {
      ...
      if (showConnectionDialog) {
        SshToolsConnectionProfile newProfile = ( (
            SshToolsApplicationClientPanel) applicationPanel)
    ...
    if (!host.equals("")) {
      ...
      if (!authenticationMethod.equals("")) {
        try {
          ...
          SshAuthenticationClient authClient = SshAuthenticationClientFactory
              .newInstance(authenticationMethod, profile);
